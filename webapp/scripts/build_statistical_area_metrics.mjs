...
const sourceGeojson = "data/webapp_vectors/districts_enriched_2026.geojson";
const outputPath = "src/data/statisticalAreas.js";

const rasters = {
  uhei: "data/webapp_rasters/UHEI_2026_summer_30m.tif",
  lst: "data/webapp_rasters/LST_2026_summer_30m.tif",
  anomaly: "data/webapp_rasters/anomaly_2026_summer_30m.tif",
  hotspot: "data/webapp_rasters/hotspot_temporal_2026_summer_30m.tif",
  persistence: "data/webapp_rasters/hotspot_temporal_persistence_2013_2026.tif",
};
...
// Measure all pixels in a district once, rather than averaging area means.
const districtDefinitions = [
  ["san-donato", "San Donato-San Vitale"],
  ["navile", "Navile"],
...
const districts = districtDefinitions.map(([id, name]) => {
  const members = geojson.features.filter((feature) =>
    normalizeName(feature.properties.quartiere) === normalizeName(name));
  if (!members.length) throw new Error(`No geometry for district ${name}`);
  const geometry = {
    type: "MultiPolygon",
    coordinates: members.flatMap((feature) => feature.geometry.type === "Polygon"
      ? [feature.geometry.coordinates] : feature.geometry.coordinates),
  };
  return { id, name, ...computeFeatureStats({ geometry }, loaded) };
});
await fs.writeFile("src/data/districts.js", `// Generated by scripts/build_statistical_area_metrics.mjs.\nexport const districts = ${JSON.stringify(districts, null, 2)};\n`, "utf8");
console.log(`Wrote ${districts.length} districts to src/data/districts.js`);
